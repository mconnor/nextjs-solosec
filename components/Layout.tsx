...
const PageContainer = styled.div<IwidthHeightPxString>`
    position: static;
    overflow: hidden;
    ...
    grid-template-rows: ${props => `100px ${props.h} 1fr`};
    grid-template-areas:
        "header header header"
        "main main main"
        ". footer .";
    grid-template-rows: 10% 1fr 54px;
...
const Layout: React.FC<Props> = ({ children, pageIndex, title = 'Solosec IVA', foreGroundArt, noBgArt = false, bgArt, section = '' }) => {
    ...
    const { ipadHeightPx, ipadWidthPx, ipadMainSectionHeightPx} = useDeviceDimensions();
